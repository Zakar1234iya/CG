...
{
	// Portable implementation replacing x86 inline assembly
	const float scale = 255.0f;
	
	// Clamp and convert each component
	unsigned int r = static_cast<unsigned int>(WWMath::Clamp(color.X, 0.0f, 1.0f) * scale);
	unsigned int g = static_cast<unsigned int>(WWMath::Clamp(color.Y, 0.0f, 1.0f) * scale);
	unsigned int b = static_cast<unsigned int>(WWMath::Clamp(color.Z, 0.0f, 1.0f) * scale);
	unsigned int a = static_cast<unsigned int>(WWMath::Clamp(alpha, 0.0f, 1.0f) * scale);
	
	// Pack into ARGB format (AA RR GG BB)
...
WWINLINE void DX8Wrapper::Clamp_Color(Vector4& color)
{
	// Portable implementation replacing x86 inline assembly
	for (int i = 0; i < 4; ++i) {
		color[i] = WWMath::Clamp(color[i], 0.0f, 1.0f);
	}
}
