...
void __cdecl ThreadClass::Internal_Thread_Function(void* params)
{
        ...
        tc->running=true;
        tc->Thread_Function();
        // WARNING: Data race - handle is written from worker thread without synchronization.
        // This is safe only because the main thread reads handle only after the thread terminates.
        tc->handle=0;
}

#ifdef _UNIX
static void* Internal_Thread_Function_Posix(void* params)
{
        ThreadClass* tc = reinterpret_cast<ThreadClass*>(params);
        tc->running = true;
        tc->Thread_Function();
...
void ThreadClass::Execute()
{
        ...
        #ifdef _UNIX
                pthread_t thread_id;
                int result = pthread_create(&thread_id, NULL, Internal_Thread_Function_Posix, this);
                if (result == 0) {
                        handle = (uintptr_t)thread_id;
                } else {
                        handle = 0;
                }
        #else
