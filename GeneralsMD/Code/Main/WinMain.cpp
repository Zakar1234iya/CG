...
// to trap a different device change (for instance, removal of a mouse) - jkmcd
        ...
        {
                RELEASE_CRASH(("Uncaught exception in Main::WndProc"));
                // Don't silently swallow - at minimum log it
        }
...
static Bool initializeAppWindows( HINSTANCE hInstance, Int nCmdShow, Bool runWindowed )
{
  ...
  if (!RegisterClass( &wndClass )) {
                DEBUG_CRASH(("Could not register window class"));
                return 0;
        }
...
{
        // WARNING: This function uses static state and is NOT thread-safe (like strtok)
...
// SetWindowMode ==============================================================
void SetWindowMode(HWND hwnd, WindowMode mode, int w, int h) {
    switch (mode) {
    ...
    case WindowMode::Windowed:
        ...
        SetWindowPos(hwnd, nullptr, 100, 100, w, h, SWP_FRAMECHANGED);
        break;
