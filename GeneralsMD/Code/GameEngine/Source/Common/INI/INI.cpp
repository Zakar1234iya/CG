...
AsciiString INI::getNextQuotedAsciiString()
{
        ...
        if (token != NULL)
        {
                ...
                {       int strLen=0;
                        ...
                        {
                                strncpy(buff, &token[1], INI_MAX_CHARS_PER_LINE - 1); buff[INI_MAX_CHARS_PER_LINE - 1] = '\0';        //skip the starting quote
                        ...
                        if (!done)
                        {
                                ...
                                {
                                        strncat(buff, " ", INI_MAX_CHARS_PER_LINE - strlen(buff) - 1);
                                        strncat(buff, token, INI_MAX_CHARS_PER_LINE - strlen(buff) - 1);
                                }
                                else
...
AsciiString INI::getNextAsciiString()
{
        ...
        if (token != NULL)
        {
                ...
                else
                {
                        ...
                        if (strlen(token) > 1)
                        {
                                strncpy(buff, &token[1], INI_MAX_CHARS_PER_LINE - 1); buff[INI_MAX_CHARS_PER_LINE - 1] = '\0';
                        } 
