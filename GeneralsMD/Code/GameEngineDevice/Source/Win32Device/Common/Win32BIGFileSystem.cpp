...
//      }
        ...
        for (Int i = 0; i < numLittleFiles; ++i) {
                ...
                // Fixed: Throttled logging — log every 5000 entries instead of per-entry to avoid 45-min Debug startup
                if ((i % 5000) == 0) {
                        DEBUG_LOG(("Win32BIGFileSystem::openArchiveFile - processed %d/%d files in %s\n", i, numLittleFiles, fileInfo->m_archiveFilename.str()));
                }
